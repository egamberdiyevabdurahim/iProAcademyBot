...
def insert_aop_panic_query(name_uz: str, name_ru: str, name_en: str, code: str, user_id: int, photo=None) -> None:
    """
    Inserts a new AOP panic notification into the aop_panic table.
    """
    query = """
    INSERT INTO aop_panic (name_uz, name_ru, name_en, code, user_id, photo)
    VALUES (%s, %s, %s, %s, %s, %s)
    """
...
def update_aop_panic_query(id_of: int, new_name_uz: str, new_name_ru: str, new_name_en: str, code: str, new_photo=None) -> None:
    """
    Updates an AOP panic notification.
    """
    query = """
    UPDATE aop_panic
    SET name_uz=%s, name_ru=%s, name_en=%s, code=%s, photo=%s, updated_at=%s
    WHERE id=%s
    """
    execute_query(query, (new_name_uz, new_name_ru, new_name_en, code, new_photo, datetime.now(), id_of))
    return None
