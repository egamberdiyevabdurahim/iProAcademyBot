...
async def activity_maker(message: Message):
    user_id_data = get_user_by_telegram_id_query(message.from_user.id)['id']
    insert_activity_query(text=message.text, user_id=user_id_data)
